...
public interface AccountMapper {
    ...
    default Account restFromId(long id) {
        return toRest(ApplicationContextProvider.getApplicationContext().getBean(AccountsDatabaseMapper.class).findById(id));
    }
    ...
    default List<ru.jawaprog.test_task_mts.Account> fromContractId(long id) {
        return toSoap(ApplicationContextProvider.getApplicationContext().getBean(AccountsDatabaseMapper.class).findAccountsByContractId(id));
    }
    ...
    @Mapping(target = "contractId", expression = "java(account.getContract()==null?null:account.getContract().getId())")
